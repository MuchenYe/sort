...
color_dist = {'red': {'L': np.array([156, 160, 70]), 'H': np.array([180, 255, 255])},
              'green': {'L': np.array([20, 116, 113]), 'H': np.array([77, 255, 255])},
...
def get_lane_info(input_roi_edges):
    ...
    if lines is not None:
        ...
        if len(left_lines) > 0:
            ...
            left_results = least_squares_fit(left_points, 0, input_roi_edges.shape[0])
        ...
        if len(right_lines) > 0:
            ...
            right_results = least_squares_fit(right_points, 0, input_roi_edges.shape[0])
...
def get_line(in_frame_process):
    ...
    if L_find_state[130]==1 and L_find_state[129]==1 and L_find_state[128]==1 :
        L_start_point = (int)((left_line[130] + left_line[129] + left_line[128]) / 3) + 50
    else:
        L_start_point = find_mid

    #处理右线起点
    if R_find_state[130]==1 and R_find_state[129]==1 and R_find_state[128]==1 :
        R_start_point = (int)((right_line[130] + right_line[129] + right_line[128]) / 3) - 50
    else:
...
#-------------------------------------------------------------------------------
    ...
    #jiaodian
    if flag_side_walk==True and lose_left_line_state>50 and lose_right_line_state>50:
        k =0 
        jiaodian_y = 0
        jiaodian_x = 0
        print("jiaodian")
...
def visual_ctrl(in_image, twist, cls_id, area):
    ...
    elif flag_line_mode==2:                            # 处理图像，输出对应红色线的二值化图，使用模式2
    ...
    if twist.linear.x == 0.25:
        ...
        twist.angular.z = 0.015* turn_error 

    # twist.angular.z = limit_val(twist.angular.z,2.0,-2.0)
    if flag_limit_on:
        twist.linear.x = 0.25 
    else: 
        twist.linear.x = 0.35 #给出基础速度
    #处理完正常巡线，进行标识处理
    '''
            0               1           2           3               4                   5               6
    ...
    #     if int(cls_id)==2 or int(cls_id)==3:
    #         print('cat:', categories[cls_id])
    #         print('area:', area)
    #         if area > 16000:
    #             twist.linear.x = 0.0
    #             twist.angular.z = 0.0
    ...
    # if flag_turn_left==True:
    #     time_count = time_count + 1
    #     if time_count>0 and time_count<=30:
    #         twist.linear.x = 0.2
    #         twist.angular.z = 0.0
    #     if time_count>30 and time_count<=55:
    #         twist.linear.x = 0.0
    #         twist.angular.z = 1.2
    #     if time_count>55:
    #         twist.linear.x = 0.0
    ...
    # if flag_turn_left==True:
        ...
        #     if time_count > 2 and time_count < 3:
        #         twist.linear.x = 0.0
        #         twist.angular.z = 0.0
        #     elif time_count > 3:
        #         event_state = 2
    ...
    #         twist.linear.x = 0.2
    #         twist.angular.z = 0.0
    #     if time_count>5 and time_count<=25:
    #         twist.linear.x = 0.3
    #         twist.angular.z = -0.6
    #     if time_count>25:
    #         twist.linear.x = 0.3
    ...
    # if int(cls_id)==6 and area>1500 and flag_side_walk_once==0:
    ...
    if int(cls_id==5) and area>7300 and flag_limit_once==0:
        flag_limit_on=True
        flag_limit_once = 1
        # flag_line_mode = 1
    ...
    # if int(cls_id==1) and area>2500:
    #     flag_branch = True
    # if int(cls_id==4) and area>6000:
    #     flag_limit_off=True 
    ...
    #         print(time_count)
    #         if lose_right_line_state > 30 and time_count>4:
    #             flag_line_mode = 0
...
def main():
    ...
    # 加载yolo模型
    PLUGIN_LIBRARY = "/home/jetson/catkin_ws/src/serial_pkg/scripts/Orin_yolov5TRT/build/libmyplugins.so"
    engine_file_path = "/home/jetson/catkin_ws/src/serial_pkg/scripts/Orin_yolov5TRT/build/yolov5s.engine"
    ...
    while True:
        ...
        if flag_limit_off==True or flag_light==True:
            light_image = img[100:380, 0:320, :]  # 180x200
        ...
        # print('green:', green_area)
            ...
            #红灯事件预备
            if red_area > 2500 and red_area<4000:
                flag_limit_off = False
                flag_light = True
                start_time = time.time()
                twist.linear.x = 0.6
                twist.angular.z = limit_val(twist.angular.z, 0,-2)
                send_data(twist.linear.x,twist.angular.z)
            #红灯事件执行
            elif red_area > 4000:
                end_time = time.time()
