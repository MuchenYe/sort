...
namespace road_ns
{
	...
	void road_image::image_process(cv::Mat &input_img)
	{
		...
		// cv::resizeWindow("pic", 780, 480);
		// cv::imshow("pic", input_img);
		// cv::waitKey(1);
		cv::GaussianBlur(input_img, input_img, cv::Size(3, 3), 0, 0);
		// cv::medianBlur(input_img, input_img, 3);
		cv::cvtColor(input_img, input_img, cv::COLOR_BGR2HSV);
	...
	void road_image::Calc_Speed(cv::Mat *frame, bool show_image)
	{
		...
		cv::HoughLinesP(*frame, lines, 1, CV_PI / 180, 15, 37, h); // 15 37 60
		...
		if (lines.size())
		{
			...
			if (road_state == Left_Turn && left_lines.size() == 0 && right_lines.size() == 0) // T
			{
		...
		// int l_show=l1.x - r1.x;
		// int r_show=l2.x - r2.x;
		//  std::cout << "l_show:" <<l_show << std::endl;
		//  std::cout << "r_show:" <<r_show << std::endl;
		std::cout << "l1:" << l1.x << std::endl;
		std::cout << "r1:" << r1.x << std::endl;
		std::cout << "l2:" << l2.x << std::endl;
		std::cout << "r2:" << r2.x << std::endl;
		int narrow_d = 170;
		int wide_d = 320;

		if (left_points.size() == 0 && right_points.size() > 0) // lose left line
		{
			if (road_state != Limit_Rate_On) // 避免左转
			{
				// turn_pwm =1200;// TURN_PWM_LEFT;
				offset = (r1.x - 220) /*( (r1.x * 2 - narrow_d) / 2 + (r2.x * 2 - wide_d) / 2) / 2 - w / 2*/;
				std::cout << "_______________left not limit_____________" << std::endl;
				// return;
			}
			else
			{
		...
		else if (left_points.size() > 0 && right_points.size() == 0) // lose right line
		{
			if (road_state != SideWalk)
			{
				// turn_pwm = TURN_PWM_RIGHT;
				offset = l1.x - 90; /* ((l1.x * 2 + narrow_d) / 2 + (l2.x * 2 + wide_d) / 2) / 2 - w / 2;*/
				std::cout << "______________right not Sidewalk____________" << std::endl;
				// return;
			}
			else
			{
		...
		if (road_state == SideWalk) // 给人行横道前的路转向微小量滤波
			angle_pid.out = Average_PWM(angle_pid.out, 10);
		else if (road_state == Ramp) // 给上坡前的转向微小量滤波
		...
		// cv::namedWindow("way", 0);
		// cv::resizeWindow("way", 780, 480);
		// cv::imshow("way",way) ;
